...
function generateMessageForMulticallHandler(
  ...
  const depositFunction = "function supply(address asset, uint256 amount, address onBehalfOf, uint16 referralCode)";
  ...
  const depositCalldata = aaveInterface.encodeFunctionData("supply", [depositCurrency, depositAmount, userAddress, aaveReferralCode]);
...
async function getSuggestedFees(
  ...
  if (message) {
    const messageHex = message.startsWith('0x') ? message : `0x${Buffer.from(message).toString('hex')}`;
...
async function depositUSDCToAaveOnArbitrum() {
  try {
    const {
      ...
      ACROSS_BRIDGE_ADDRESS_BASE,
      MULTICALL_HANDLER_ADDRESS_ARBITRUM,
      BASE_CHAIN_ID,
      ARBITRUM_CHAIN_ID,
      BASE_RPC_URL
    } = process.env;
    ...
    const usdcContract = new ethers.Contract(USDC_ADDRESS_BASE, ERC20_ABI, wallet);
    ...
    console.log("Suggested fees received:", JSON.stringify(suggestedFees, null, 2));
    const outputAmount = depositAmount - ethers.toBigInt(suggestedFees.relayFeeTotal);
    ...
    try {
      const approveTx = await usdcContract.approve(ACROSS_BRIDGE_ADDRESS_BASE, depositAmount, {
    ...
    const encodedParams = abiCoder.encode(
      ...
      [
        ...
        MULTICALL_HANDLER_ADDRESS_ARBITRUM,   // recipient (multicall handler)
    ...
    const finalData = "0x" + manualData.substring(2) + DELIMITER + UNIQUE_IDENTIFIER;
    
    console.log("Manually constructed data with selector:", finalData.substring(0, 10));
    console.log("Expected selector:", DEPOSIT_V3_SELECTOR);
    console.log("Data includes unique identifier:", DELIMITER + UNIQUE_IDENTIFIER);
    const feeData = await provider.getFeeData();
    
    // Create transaction with the final data
    const tx = {
      to: ACROSS_BRIDGE_ADDRESS_BASE,
      data: finalData,
